...
建模路线：
  ...
  ⑥ 可视化 + 写回 HBase（真实接入虚拟机 192.168.92.128）
...
def main():
    ...
    if transactions is None:
        ...
        transactions, raw_df = load_transactions(
            ...
            skill_col=getattr(config, "SKILL_COL", "skills"),
        )
        print(f"数据源: {source}（{config.SKILLS_FILE}）")
    else:
    ...
    print("  部署到虚拟机 Spring Boot static 后访问 http://192.168.92.128:8080/")
    print("=" * 60)
