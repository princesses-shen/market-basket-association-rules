...
function drawFreq(list) {
  ...
  freqEl.setOption({
    title: { text: 'Top 频繁项集', left: 'center', textStyle: { color: '#4ade80', fontSize: 14 } },
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' },
               formatter: function (p) { return p[0].name + '<br/>support=' + p[0].value; } },
    grid: { left: '28%', right: '12%', top: 40 },
    xAxis: { type: 'value', name: 'support', axisLabel: { color: '#ccc' }, nameTextStyle: { color: '#ccc' } },
...
function drawScatter(rules) {
  var ruleMap = {};
  var data = rules.map(function (r, i) { ruleMap[i] = r.rule || (r.antecedents + ' => ' + r.consequents); return [Number(r.confidence), Number(r.lift), Number(r.support), i]; });
  scatterEl.setOption({
    title: { text: '置信度 vs 提升度', left: 'center', textStyle: { color: '#4ade80', fontSize: 14 } },
    ...
    xAxis: { type: 'value', name: 'confidence', min: 0, max: 1.0, nameLocation: 'middle', nameGap: 28,
             axisLabel: { color: '#ccc', formatter: function (v) { return v.toFixed(1); } }, nameTextStyle: { color: '#ccc' } },
    yAxis: { type: 'value', name: 'lift', axisLabel: { color: '#ccc' }, nameTextStyle: { color: '#ccc' } },
    visualMap: { min: 1, max: 15, dimension: 1, calculable: true, orient: 'vertical',
                 right: 10, top: 50, textStyle: { color: '#ccc' },
                 inRange: { color: ['#f5d4d4', '#c23531'] },
                 text: ['lift高', 'lift低'], itemHeight: 120 },
...
function drawRules(rules) {
  ...
  var labels = top.map(function(r) { return r.rule || (r.antecedents + ' => ' + r.consequents); });
  ruleEl.setOption({
    title: { text: 'Top 强规则', left: 'center', textStyle: { color: '#4ade80', fontSize: 14 } },
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' }, formatter: function (p) {
      return labels[p[0].dataIndex] + '<br/>lift=' + p[0].value.toFixed(3); } },
    grid: { left: '38%', right: '12%', top: 40 },
    xAxis: { type: 'value', name: 'lift', axisLabel: { color: '#ccc' }, nameTextStyle: { color: '#ccc' } },
    yAxis: { type: 'category', data: labels, axisLabel: { fontSize: 11, color: '#ccc' } },
    series: [{ type: 'bar', data: top.map(function (r) { return Number(r.lift); }),
